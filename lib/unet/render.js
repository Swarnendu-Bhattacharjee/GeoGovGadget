// Canvas ports of the render() step in ml/building_detector/infer_unet.py, so
// the "Rendered" and "Confidence" views look the same whichever engine ran.

// cv2.COLORMAP_INFERNO, RGB triples. Baked in rather than approximated: the
// heatmap is read as a confidence readout, and an eyeballed gradient would
// misreport which parcels the model was actually unsure about.
...
/**
 * The annotated overlay: a translucent fill plus a solid outline per parcel.
 *
 * OpenCV builds this with addWeighted(overlay, 1.0, fill, 0.32, 0) — an
 * *additive* blend against a black-backed fill layer, not alpha compositing.
 * Canvas's "lighter" operator is the additive equivalent, so the fill colour is
 * pre-scaled by 0.32 and added, which keeps bright roofs from washing out the
 * way straight alpha would.
 */
export function renderAnnotated(imageBitmapSource, width, height, polygons) {
  const canvas = makeCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ctx.drawImage(imageBitmapSource, 0, 0, width, height);

  ctx.globalCompositeOperation = "lighter";
  // BGR (255, 190, 60) in the Python source -> RGB (60, 190, 255), times 0.32.
  ctx.fillStyle = `rgb(${Math.round(60 * 0.32)}, ${Math.round(190 * 0.32)}, ${Math.round(255 * 0.32)})`;
  for (const p of polygons) {
    polygonPath(ctx, p.points);
    ctx.fill();
  }

  ctx.globalCompositeOperation = "source-over";
  ctx.strokeStyle = "rgb(90, 210, 255)"; // BGR (255, 210, 90)
  ctx.lineWidth = 2;
  ctx.lineJoin = "round";
  for (const p of polygons) {
    polygonPath(ctx, p.points);
...
/** cv2.applyColorMap(prob * 255, COLORMAP_INFERNO). */
export function renderHeatmap(prob, width, height) {
  const canvas = makeCanvas(width, height);
  const ctx = canvas.getContext("2d");
  const image = ctx.createImageData(width, height);
  for (let i = 0; i < prob.length; i++) {
    // Truncating, not rounding — .astype(np.uint8) truncates.
    const v = Math.min(255, Math.max(0, Math.trunc(prob[i] * 255))) * 3;
    image.data[i * 4] = INFERNO[v];
    image.data[i * 4 + 1] = INFERNO[v + 1];
    image.data[i * 4 + 2] = INFERNO[v + 2];
