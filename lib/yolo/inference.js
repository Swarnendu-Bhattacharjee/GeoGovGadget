...
export async function predictFull(session, rgba, width, height, onProgress, conf = 0.25) {
  // Per-stage timing, on by default in the browser console. The decode has
  // already produced two pathological slowdowns; measuring beats guessing.
  const DEBUG = typeof window !== "undefined" && window.__YOLO_DEBUG !== false;
  const t = { setup: 0, resize: 0, forward: 0, decode: 0, paint: 0, dets: 0 };
  const clock = () => performance.now();
  let mark = clock();
  const lap = (k) => { const n = clock(); t[k] += n - mark; mark = n; };
  ...
  for (const y of ys) {
    for (const x of xs) {
      ...
      const S = IMGSZ / PROTO;
      const decoded = decodeTile(out0, protos, conf, 0.45);
      t.dets += decoded.length;
      lap("decode");
      ...
      if (DEBUG) {
        console.log(
          `[yolo] tile ${done}/${total} dets=${decoded.length} ` +
          `resize=${t.resize | 0}ms forward=${t.forward | 0}ms ` +
          `decode=${t.decode | 0}ms paint=${t.paint | 0}ms`
        );
      }
  ...
  if (DEBUG) {
    console.log(
      `[yolo] TOTAL tiles=${total} dets=${t.dets} setup=${t.setup | 0}ms ` +
      `resize=${t.resize | 0}ms forward=${t.forward | 0}ms decode=${t.decode | 0}ms ` +
      `paint=${t.paint | 0}ms`
    );
  }
  return canvas;
